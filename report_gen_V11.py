...
A_I_dict={}
filename=dir_nam+'/AUTH_INTRO'+str(DataName)+'.json' # monatl.  Autor und Text zum report'
try:  
    with open(filename, "r") as read_file:
...
filename= dir_nam+'/AUTH_INTRO_DEFAULT'+'.json'  # wenn nicht Default Autor und Text 
try : 
    with open(filename, "r") as read_file:
        A_I_dict = json.loads(read_file)
except IOError as Fehler:
...
intro_text=A_I_dict.get('Intro','This report provides analysis and presentation of measured data for the storage system '+HEAD_dict.get('Storage_System_Name')                        )
